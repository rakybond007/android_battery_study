...
def runtime_log_with_voltage(frequency, log_num):
    global Finish_flag
    th_current = Thread(target=log_current_pc, args=(frequency, log_num,))
    th_voltage = Thread(target=log_voltage_pc, args=(frequency, log_num,))
    th_current.start()
    th_voltage.start()
    th_voltage.join()
    th_current.join()
...
for i in range(3):
    ...
    th_log = Thread(target=runtime_log, args=(0.1, 16+i,))
    th_experiment.start()
    th_log.start()
    th_log.join()
    th_experiment.join()
